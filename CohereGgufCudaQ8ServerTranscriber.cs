...
namespace WhisperInk
{
    ...
    /// running on CUDA with the Q8_0 quantised model. Port 8768. Lazy-started on first call.
    ///
    ...
    /// </summary>
    public sealed class CohereGgufCudaQ8ServerTranscriber : IDisposable
    {
        private static readonly string ModelFolder = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
            ".WhisperInk", "cohere-gguf-cuda-q8");

        private static readonly string ExePath      = Path.Combine(ModelFolder, "crispasr.exe");
        private static readonly string DefaultModel = Path.Combine(ModelFolder, "cohere-transcribe-q8_0.gguf");

        private const string ServerHost = "127.0.0.1";
        private const int    ServerPort = 8768;
        ...
        public CohereGgufCudaQ8ServerTranscriber() : this(ExePath, DefaultModel) { }

        public CohereGgufCudaQ8ServerTranscriber(string exePath, string modelPath, int threads = 0)
        {
            _exePath   = exePath;

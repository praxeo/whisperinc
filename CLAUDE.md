...
3. **Runtime — `%APPDATA%\.WhisperInk\`** — hardcoded deploy target. Contains:
   - `config.json`, `debug.log` (this session) + `debug.previous.log` (the one before), `history.json` — app state
   - `unsent\` — `take-*.wav` + `.json` for every take not yet delivered (see [Never lose a dictation](#never-lose-a-dictation-loud-failures-2026-09))
...
| File | Purpose |
|------|---------|
| `MainWindow.xaml.cs` | Orchestration: recording state machine (`_recState` tri-state via `Interlocked`), transcription dispatch (one factory call, under the take's deadline), verified-target delivery, the unsent-take retry flow, config load/save, the shared `BuildAppMenu()` tree, and the shared cloud `HttpClient` (9-min idle pool, TCP keep-alive, connection pre-warm at key-press) and the streamed-upload hand-off (`BeginStreamedTake`; see [Cloud connections & latency](#cloud-connections--latency-2026-09)). ~2170 LOC. |
...
| `MicCapture.cs` | Owns the microphone. Holds the device open between dictations ("warm mic") and streams every buffer into a `PreRollRing`, so `BeginCapture()` starts a recording that already contains the last ~400 ms. No device open on press, no teardown on release. Also hosts `Measure()` (peak + RMS) for the silence gate, and flags a capture whose device died mid-take (`LastCaptureInterrupted`, plus an `onCaptureLost` callback raised while still recording). `BeginCapture(sink)` hands an optional sink the same bytes as the WAV, in the same order and under the same lock (the pre-roll, then each buffer); a sink that throws is detached and the WAV never depends on it. |
...
| `TextInjector.cs` | Synthetic text delivery: clipboard paste-with-restore (batch), `CopyToClipboard()` (no paste, no restore — for text that can't safely be pasted), selection grab via Ctrl+C, `ReleaseAllModifierKeys()`. Owns pending clipboard-restore state. |
| `TranscriptionDeadline.cs` | Per-take deadline, scaled to the recording: cloud 20 s + 20 s per minute of audio (cap 15 min), local/loopback 180 s + 1× the audio (cap 2 h). Every transcription runs under it; the HttpClients' own timeouts are only a longer backstop. |
| `StreamedTranscription.cs` | An ElevenLabs take's upload that starts at the key-**press**: the ordinary request, field for field, with the audio part sent chunked as raw PCM (`file_format=pcm_s16le_16`) while it is spoken, so the release waits only for the transcript. Used only when it carried exactly the WAV's bytes; any other failure but the deadline falls back to the WAV upload; a discarded take cancels it mid-body (nothing billed). See [Cloud connections & latency](#cloud-connections--latency-2026-09). |
| `UnsentTakes.cs` | Journals every take's WAV (+ JSON sidecar) to `%APPDATA%\.WhisperInk\unsent\` before it is sent; deletes it on delivery, keeps it with a reason on any failure, recovers takes left "pending" by a crash, bounds retention (14 days / 50 takes). Backs the ↻ Unsent dictations menu. |
| `TranscriptCoverage.cs` | The incomplete-transcript check (ported from elevenlabs-web's `coverageShortfall`): last transcribed word vs. the last speech in the WAV, slack max(20 s, 25 %). |
...
| `HttpTranscriber.cs` | OpenAI-compatible multipart POST. Covers Mistral batch, OpenAI Whisper, Cohere v2 cloud, ElevenLabs Scribe v2 (gated on `ApiProvider.IsElevenLabs`: `language_code`, temperature 0, one speaker, word timestamps, keyterms, `tag_audio_events`/`no_verbatim`, plus elevenlabs-web's transcript cleanup and the word timing for `TranscriptCoverage`), Qwen3-ASR, and any user-added cloud provider. |
...
Captures via `MicCapture`, journals the take (`UnsentTakes`), POSTs the in-memory WAV under a per-take deadline (for ElevenLabs the upload is already streamed during the hold, and the WAV is the fallback), pastes with a leading space into the window the dictation started in (only if it is still in front). Works with every provider. `~/Documents/MyRecordings/temp_audio.wav` is written **after** capture as a fire-and-forget replay/debug artifact only — it is no longer in the recording path (that folder is OneDrive-synced on the desktop, so a sync stall must never be able to stall a dictation). The durable copy of a take is the journal in `%APPDATA%\.WhisperInk\unsent\`, which is not synced.
...
**Empty text is judged by the captured level (2026-09).** A provider that answers with no text for a take *below* the speech floor (the silence threshold, or its 0.003 default when the gate is disabled) gets the quiet `Dismissed` blip — nothing was said. *Above* the floor it is a **failure**: `Error` tone, "No text!" status, and an `[error] … returned NO TEXT for …ms of audio at RMS …` line. Real sound went in and nothing came out, which is exactly how a broken backend presents (a CrispASR server missing a DLL answers with empty text); when this was the quiet blip, every dictation could fail that way unnoticed. The silence gate still drops genuinely silent takes before any API call, so this only fires on audio the gate judged to be speech — a cough-only press can buzz, and the fix for that is the threshold, never muting the failure.

**Tuning knobs** (all in `config.json`, all clamped on load): `WarmMicEnabled`, `WarmMicIdleSeconds` (0 = hold while running), `PreRollMs`, `PostRollMs`, `MinHoldMs`, `SilenceThreshold` (0 = disable the gate), `StreamUpload` (default true; false = ElevenLabs takes upload as a WAV after release, as before 2026-09).

### Never lose a dictation, loud failures (2026-09)

Ported from the reliability work in `praxeo/elevenlabs-web`, the clinical dictation web app — **read-only reference**, never changed for this (`worker.js`: `handleTranscribeBatch`, `cleanTranscript`, `coverageShortfall`; `keyterms.js`). Checked 2026-09-23: the deployed `eleven` worker (version 208) matches that repo's `main` (`fa2b7b1`) and the local clone at `C:\elevenlabs-web`.

**Deadlines scale with the take.** Every transcription runs under `TranscriptionDeadline.For(provider, audioSeconds)` — cloud 20 s + 20 s per minute of audio (cap 15 min), local or loopback 180 s + 1× the audio (cap 2 h) — passed as the `CancellationToken` to `ITranscriber.TranscribeAsync`. The flat limits it replaced were length limits in disguise: the shared cloud `HttpClient` failed anything past **15 s** of upload + inference, CrispASR's client anything past **120 s**, Soniox's poll anything past **120 s**. The clients now carry `TranscriptionDeadline.HttpBackstop` (2 h 10 min), longer than any deadline, so the token always governs. Measured basis (desktop `debug.log`, ~100 takes): ElevenLabs ≈ 0.45 s + 1.13 s per minute of audio; warm Voxtral 4B ≈ 0.11 s per second of audio; the first local take after a switch pays 1.5–3 s of server start; the WAV is 1.8 MB per minute. When a deadline fires: `[error] … did not finish within its Ns deadline`, Error tone, "Timed out — saved ↻", a tray balloon, and the take is kept. Each cloud transcriber's cancel arm now tells the take's deadline (`catch (OperationCanceledException) when (ct.IsCancellationRequested)` → "stopped at the take's deadline") apart from the HttpClient's own timeout ("HTTP request timed out") — both used to log as "cancelled". A CrispASR request cancelled mid-inference also marks the server not-ready, so the next take gets a fresh server instead of queueing behind the wedged inference. Google Chirp 3 keeps its own 30 s client (its sync API refuses audio over ~60 s anyway).

**Every take is journaled.** `UnsentTakes.Begin` writes the WAV + a JSON sidecar to `%APPDATA%\.WhisperInk\unsent\` (off the dictation path) *before* the send; `Delivered` deletes them, `Keep` records why (`failed` / `incomplete`); an exception anywhere in the stop path still keeps the take. At startup `Recover()` turns anything still `pending` (the app closed, crashed or the PC restarted mid-take) into `interrupted`, prunes to 14 days / 50 takes, and a tray balloon says how many are waiting. **↻ Unsent dictations** (tray + bar menu) lists them, each with *Retry with {active provider}* and *Retry on {local model} (local fallback)* for every local preset whose exe + GGUF are on disk. A retry puts the text on the **clipboard** — never pastes, the original window is long gone — adds it to History, deletes the take on success, and flags a fallback with the Warn tone and a balloon naming the model. The fallback's server is dropped afterwards so a model nobody switched to doesn't stay resident. Retries go through the same `_recState` gate as a dictation. This is clinical audio, which is why the journal lives in `%APPDATA%` (not OneDrive-synced like `MyRecordings`) and retention is bounded.

**The previous session's log survives a restart.** `debug.log` is moved to `debug.previous.log` at launch; it used to be truncated, and the usual reaction to a failure is a restart, which destroyed the evidence. The support bundle includes both tails.

**Loud failures.**
- **The paste target is verified.** `SetForegroundWindow`'s result used to be ignored, so when Windows refused the switch the Ctrl+V landed in whatever had focus — the wrong app, or the wrong chart. `DeliverAsync` pastes only when the window the take started in is really in front (allowing ~150 ms for a granted switch to land); otherwise `TextInjector.CopyToClipboard` leaves the text for the user, with the Warn tone and a balloon. The target is captured when a take actually *starts*: the hook callback used to overwrite it on every press, so a press made during the previous take's transcription re-pointed that take's paste. Log lines carry window handles only — titles can carry patient names.
- **Warn tone** (660 Hz, two pulses) = "delivered, but check it": a possibly-incomplete transcript, a mic that cut out mid-take, text left on the clipboard, a local-fallback retry. `Success` must only ever mean "all of it, where you were typing".
- **Incomplete transcript** (`TranscriptCoverage`; ElevenLabs only so far — the one provider asked for word timestamps): warns when the last transcribed word ends more than max(20 s, 25 % of the take) before the last speech in the WAV (30 ms frames over 0.01 RMS, three in a row), or `audio_duration_secs` is that far short of the recording. Baselined on the last *speech*, never the hold, so keeping the key down after finishing can't warn; missing timing can only mean "no warning". The text is still pasted (it's real), with Warn + "⚠ Check the end!" + a balloon, and the take is kept for a retry.
- **Mic cut out mid-take.** `MicCapture` flags a device that died during the capture (unplug, driver reset) and raises `onCaptureLost` immediately — Error tone + "🎙 MIC LOST" while the user is still talking. A mic that silently stopped delivering buffers shows up as a capture ≥ 1 s *shorter* than the hold (normally it's longer, by the pre-roll); the device is then reopened, because a stalled one never recovers and its frozen pre-roll would seed the next take. Either way the partial text goes out with Warn + "⚠ Mic cut out!"; a cut-out take that came back silent is an Error ("Mic lost!"), never the quiet silence blip. It used to be handed on as a complete take.
- **No signal at all.** A take of pure digital zeros (peak 0) is a muted or dead input — the Windows privacy switch, a hardware mute — not a pause: Error + "No mic signal!". A real mic always has a noise floor (0.0006–0.0012 RMS measured here).
- **A press while busy** — the previous take or a retry still transcribing — used to be ignored silently. It now gets the Dismissed blip and a `[skip]` line; with deadlines that scale to long takes, that wait can be long enough to start talking into nothing.

**Tested by** `_scratch\crisp-harness` (`make-speech.ps1`, then `dotnet run`; `dotnet run -- fast` skips the real-crispasr runs): the deadline values; the ElevenLabs request shape and response parsing against a fake server, including a 16 s answer that the old 15 s client failed; the coverage check on synthetic WAVs; the journal (deliver, keep, crash recovery, orphan WAV, retention); provider resolution, preset key inheritance and the repaired Granite glob; the streamed upload against the fake server (chunked, byte-exact PCM, the ordinary fields in order, and the short-stream / discarded-take / HTTP-error / deadline / 5-minute paths); and CrispASR deadline-mid-inference + restart on the real binary. The live-API side is `_scratch\scribe-latency` (modes listed in its header; every run bills real audio).

### Cloud connections & latency (2026-09)

Measured live on 2026-09-23 against ElevenLabs Scribe with `_scratch\scribe-latency`, which links the shipping `HttpTranscriber`, using the JFK sample and a TTS clip. `api.elevenlabs.io` sits behind Google's front end (edge ping 8–11 ms), and requests are served from `x-region: us-central1`. The desktop uploads at ~2.5 MB/s.

| What | Measured |
|---|---|
| Warm connection, 1–7 s take | ~340–400 ms from release to text. A longer take adds almost exactly its upload time; server time is nearly flat |
| Fresh connection (DNS + TCP + TLS + a cold congestion window) | +166 ms median on a 4 s take |
| .NET's default pool (1-minute idle timeout) | Dropped the connection after every gap over 60 s. That evening's log had 9 of 14 real takes go out cold (446–1,117 ms, against 341–383 ms warm) |
| Connection opened at key-press (pre-warm) | Recovers ~90 of the 166 ms. The rest is TCP slow start on the upload |
| Pooled connection with a 10-minute idle timeout | Reused after 75 s (3/3), 5 min (1/1) and 9 min (1/1) gaps; ElevenLabs' side kept it open |
| `scribe_v2_medical` vs `scribe_v2` | 88–129 ms faster (warm, same 249 keyterms) |
| 249 keyterms vs none | ~0 ms on a 4 s take, +110–140 ms on 11–34 s takes |
| `file_format=pcm_s16le_16` (raw PCM instead of WAV) | No gain (+16 to +42 ms, noise), despite the docs' latency claim |
| Streaming the upload while the user talks (probe) | The API and front end accept it: chunked multipart, raw PCM, identical transcripts, and a 104 s real-time-paced upload survived. −80 to −120 ms on short takes, −950 ms on a 104 s take (n=1) |
| The shipped `StreamedTranscription` on `scribe_v2_medical`, vs pre-warm + POST at release | 4 s take: 240 vs 323 ms after a pause, 246 vs 307 ms warm. 11 s take: 307 vs 402 ms and 309 vs 393 ms (medians of 5). No fallbacks; same transcripts |
| A cancelled stream (a discarded take) | **Not billed**: the account's usage counter rose 64 for one normal request and 0 for three cancelled streams |

What shipped from this:
- **The shared cloud `HttpClient`** (`MainWindow.CreateCloudHttpClient`) keeps idle connections for 9 minutes instead of 1. Every socket gets TCP keep-alive: the first probe after 45 s idle, then every 5 s, three misses allowed. Without it, a NAT or firewall that silently forgot the idle mapping would turn the next request into a hang until the take's deadline; with it, a dead connection is noticed and dropped instead of reused. Every new connection is logged as `[net] new connection to …`, so `debug.log` shows whether a take reused one.
- **Pre-warm at key-press** (`PrewarmConnection`): a HEAD to the provider host's root, fire-and-forget, capped at 5 s, logged as `[net] pre-warmed …`. It runs for every cloud kind except Google Chirp 3, which has its own `HttpClient`, and never for local servers, and not when the take is being streamed.
- **The streamed upload** (`StreamedTranscription`, ElevenLabs only, `StreamUpload` in config.json). At the press `MainWindow.BeginStreamedTake` opens the ordinary request with its audio part left open, and `MicCapture` feeds that part the same bytes it writes to the WAV. At release the stream is ended and the transcript awaited under the take's deadline. The rules that make it safe:
  - **Byte-for-byte check.** The streamed result is used only if the stream carried exactly the WAV's PCM (`StreamedTranscription.PcmLength`). That is checked *before* the body is ended, so a short stream is cancelled rather than transcribed.
  - **Fallback.** Any failure other than the deadline, including a provider switch or settings save during the hold (the transcriber changed), sends the WAV the ordinary way under the same deadline. The WAV is still journaled first, as always.
  - **Discarded takes.** A tap or silent take cancels the stream mid-body, so the service never sees a complete request (measured: not billed).
  - **Threading.** Nothing touches the network on the UI thread (HttpClient's first request can block on proxy auto-detection, which on the press path would eat audio past the pre-roll), and the capture thread only copies into a queue.
  - **Length cap.** Past 5 minutes of audio the stream is given up; how long the front end tolerates a slow upload beyond the measured 104 s is unknown.
  - **Logging.** `[stream] …` lines plus `took …ms after release (streamed)` on the timing line.
- **The `elevenlabs-medical` preset** (see the providers table).

Not worth doing: Scribe v2 Realtime (keyterms capped at 50 terms of ≤20 characters, ElevenLabs itself says batch is more accurate, it costs more, and there is no medical variant); `file_format=pcm_s16le_16` on its own; a regional host (`api.us.elevenlabs.io` only opts out of global routing, and global routing already lands in `us-central1`).

**Billing footnote:** with more than 100 keyterms, ElevenLabs bills every request as at least 20 s, on top of the 20% keyterm surcharge, so a 3 s dictation is billed as 20 s. Getting the shared list plus `ScribeKeytermsRaw` under 100 removes that floor. Scribe Medical got all six clinical clips right with no keyterms at all, so much of the clinical part of the list may not be needed.

### Provider system

`ApiProvider` captures every knob; the cloud-vs-local split lives in `TranscriberKind`:

- **Shared HTTP/cloud knobs**: `BaseUrl`, `TranscriptionEndpoint` (override), `AuthHeaderName` (blank = Bearer, `xi-api-key` for ElevenLabs), `ModelFieldName` (`model` vs `model_id`), `TranscriptionModel`, `SupportsTranscription`, `TranscriptionTemperature` (nullable). On an `elevenlabs.io` host a blank `AuthHeaderName` resolves to `xi-api-key` and a blank `ModelFieldName` to `model_id` (`ResolvedAuthHeaderName` / `ResolvedModelField`): ElevenLabs accepts nothing else, and a hand-added Scribe Medical entry with the auth field left empty sent Bearer and got 401 "Provided authorization header was invalid" on every take (2026-09-23).
- **Bias**: one shared `AppConfig.ContextBiasTerms` list is routed to each provider's NATIVE field by `ApiProvider.BiasMechanism` (baked per provider in `CreateDefaults`; never user-set — `ResolvedBiasMechanism` falls back to the legacy `ContextBiasMode` for user-added providers, except an ElevenLabs host, which gets `elevenlabs_keyterms`). Mechanisms: `whisper_prompt` (labeled glossary in `prompt` — OpenAI, local shims), `mistral_context_bias` (comma string in `context_bias` — Mistral Voxtral batch, ≤100 terms), `elevenlabs_keyterms` (repeated `keyterms` from the shared list), `hotwords` (comma string — CrispASR local; **two very different mechanisms share this one name**: (a) a real CTC/TDT/RNNT **trie** on Parakeet, opt-in via `HotwordsBoost` and **off by default** because boosting garbles neighboring words, and (b) a **prompt splice** on the speech-LLM backends: Voxtral-3B and Qwen3-ASR append the terms to the ChatML prompt as "…may appear in the audio", and Granite (since upstream `8fad1cb9`, 2026-06-30) appends ` Keywords: <list>` to its instruction. Path (b) on `qwen3-asr-1.7b-local` is the **only local biasing in WhisperInk measured to reliably flip a hard term without collateral damage** (see below; Granite flips the same terms but rewrote a correct one); accepted-but-no-op on Cohere/Voxtral-4B), `phrase_sets` / `context_terms` / `deepgram_keyterm` / `modulate_custom_terms` / `reson8_phrases` (Google / Soniox / Deepgram / Modulate / Reson8, handled natively in their transcribers — Deepgram routes the shared list to Nova-3 `keyterm` query params, or legacy `keywords` on older models; Modulate packs it into `custom_terms` inside the JSON `config` form field, Velma 2 Multilingual batch only; Reson8 joins it into a comma-separated `phrases` query param, ≤250 terms — and is the one provider where an over-long list **degrades** accuracy per upstream rather than being merely ignored), `none`. **Cohere Transcribe v2 has no biasing field** — its old `cohere_terms`→`context_bias_terms` was a phantom the server silently dropped, now removed.
- **ElevenLabs-only**: `TagAudioEvents`, `NoVerbatim`. Keyterms come from the shared Context Bias list plus `ScribeKeytermsRaw`, the ElevenLabs-only list merged in (validated together: ≤1000 terms / <50 chars / ≤5 words, illegal chars dropped). **Long specialty lists — e.g. elevenlabs-web's `keyterms.js` medical presets — belong in `ScribeKeytermsRaw`, not the shared list**: the shared list reaches every provider, most cap it at 100 terms, and Qwen3/Voxtral splice it into the decoder prompt, where a long list dilutes (see the Qwen3 caveat below). The request shape mirrors what elevenlabs-web runs in production (`handleTranscribeBatch`): `language_code` (omitted for `auto`; a bare `language` 422s), `temperature=0` unless one is configured, `diarize=false` + `num_speakers=1`, `timestamps_granularity=word` (feeds `TranscriptCoverage`), `tag_audio_events`, `no_verbatim`, keyterms. The transcript gets elevenlabs-web's `cleanTranscript`: pause ellipses (`…`/`...`) and line breaks → spaces, runs of spaces collapsed, no space before punctuation. All of it is gated on `ApiProvider.IsElevenLabs` (`xi-api-key` auth or an `elevenlabs.io` host) — it used to be "has any custom auth header", which would have sent Scribe-only fields to, and withheld `language` from, any user-added provider authenticating with e.g. `X-API-Key`.
...
| `elevenlabs-medical` | ElevenLabs Scribe Medical | `Http` | Same request as `elevenlabs` with `model_id=scribe_v2_medical` (GA 2026-09-11, same price, batch-only). Measured 88–129 ms faster than `scribe_v2` and 6/6 on the clinical clips even without keyterms (see [Cloud connections & latency](#cloud-connections--latency-2026-09)). On first merge it takes the key, `ScribeKeytermsRaw` and output switches from `elevenlabs` |
...
| `granite-local` | Granite Speech 4.1 Local (CrispASR) | `LocalCrispAsrServer` | Port 8107, `--backend granite`, glob pinned to `granite-speech-4.1-2b-q*.gguf` (the old `granite-speech-*.gguf` loaded the 2b-plus GGUF). `hotwords` is real but **not chart-safe**: it fixes the hard terms but rewrote a correct "ureteral colic" into "ureterolithiasis", and output is all-lowercase |
...
**Which one is actually active is per-machine** — `config.json` → `ActiveProviderId`, never the `"mistral"` literal in `AppConfig`. On the desktop it has moved often: `cohere-local-q6k` and `qwen3-asr-1.7b-local` on 2026-08-29, `elevenlabs` on 2026-09-23 (switched to `elevenlabs-medical` that evening). Read the file; don't trust any sentence here. That matters when reasoning about vocabulary: Scribe v2 (and Scribe Medical) is one of the cloud providers with *real* keyterm biasing (with `google-chirp3`, `soniox`, `deepgram`, and Modulate Multilingual), so the shared Context Bias list is live on that path.

**Local biasing is no longer a dead end (2026-08-29).** The long-standing verdict — Parakeet's trie never flipped a hard term, Cohere/Granite/Voxtral-4B ignore the field (Granite no longer does; see the 2026-09-23 re-check below), and `deepgram-medical` was the only thing that could resolve `ureterolithiasis` — was true of every local preset *that existed at the time*. It is not true of `qwen3-asr-1.7b-local`. Measured on the six clips in `_scratch\biasing\clips\`, server mode, greedy:
...
**Re-checked 2026-09-23 — nothing new upstream, and Granite now biases (unsafely).** CrispASR v0.8.31–v0.8.36 (647 commits past the deployed `f632edf3`) changed nothing about biasing. `_scratch\biasing\_local_bias_ab.ps1` re-ran the six clips with the real 21-term shared list, two reps each:

| clip | Qwen3-ASR 1.7B, none → bias | Granite 4.1 2B, none → bias |
|---|---|---|
| `hematochezia_1` | "hematuria" → **hematochezia** ✅ | "hematuria" → **hematochezia** ✅ |
| `hematochezia_2` | "hematemesis" → **hematochezia** ✅ | "hematuria" → **hematochezia** ✅ |
| `ureterolithiasis` | "bursitis with edema" → **ureterolithiasis** ✅ | "ureteral atresia" → **ureterolithiasis** ✅ |
| `biliary_colic` | correct → unchanged | "gallbladder pain" → "biliary colic" |
| `ureteral_colic` | correct → unchanged | correct → **"ureterolithiasis"** ❌ |
| `neutral` | correct → unchanged | correct → unchanged |

Qwen3 holds with the real list (~160–290 ms a clip on CUDA, faster than any cloud provider). Granite reads the list since upstream `8fad1cb9`, already in the v0.8.30 deploy, which is why the old "Granite ignores it" line was wrong. But it **replaced a correctly heard term with a list term**, a clinically different finding, and the plain 4.1 2B model emits all-lowercase text. The 2b-plus GGUF (which the old loose `granite-local` glob actually loaded) was worse: with bias it wrote "hematemesis" for hematochezia. Qwen3-ASR stays the one local preset to use for vocabulary.
...
Soniox has no synchronous "POST audio → text" endpoint, so each dictation is a four-step job on `api.soniox.com/v1` (Bearer auth): `POST /files` (multipart) → `POST /transcriptions` (JSON: `model`, `file_id`, `language_hints`, optional `context.terms`) → poll `GET /transcriptions/{id}` until `status` is `completed`/`error` → `GET /transcriptions/{id}/transcript`. The transcript is a `tokens[]` array whose text carries its own leading spacing, so concatenation rebuilds the sentence (a flat `text` field is a fallback). Polling is every 400 ms; what ends a slow job is the take's deadline token (`TranscriptionDeadline`), and the poll ceiling (`CloudCap`, 15 min) only stops a job polled without one. A `finally` block best-effort `DELETE`s both the transcription and the uploaded file (even on cancel/error) so nothing accumulates under the key; each DELETE has its own 10 s budget, since the take's token may already be cancelled. Default model `stt-async-v5` (current GA as of 2026-06-11; `stt-async-v4` is deprecated, removed 2026-06-30) is user-editable, so a Soniox model rename is a config edit, not a recompile. Context-bias terms map straight onto `context.terms` (real vocabulary steering, capped at 100), so `BiasMechanism` is informational only — same approach as Google Chirp 3.

### Deepgram Listen (DeepgramTranscriber)

Deepgram is a one-shot synchronous POST — no job/poll cycle like Soniox. Each dictation `POST`s the WAV as the **raw request body** (`Content-Type: audio/wav`, no multipart) to `api.deepgram.com/v1/listen`, with auth `Authorization: Token <ApiKey>` (not Bearer) and every option carried as a **URL query parameter**: `model` (default `nova-3`, user-editable), `smart_format=true` (baked on — restores punctuation/casing/number formatting for dictation), and `language` when set (omitted for `auto`). The transcript is read from `results.channels[0].alternatives[0].transcript`. One request, bounded by the take's deadline (short clips return in ~1–3 s). Context-bias terms map onto **Nova-3 keyterm prompting** via repeated `keyterm` query params (real vocabulary steering, capped at 100); for non-Nova-3 models the transcriber falls back to the legacy `keywords` param, so biasing survives a user model swap. `BiasMechanism` (`deepgram_keyterm`) is informational only — same native-routing pattern as Google Chirp 3 / Soniox. Because it isn't OpenAI-compatible (raw body, query-param options, Token auth, nested response), it bypasses `HttpTranscriber` and has its own `TranscriberKind.Deepgram` factory branch.
...
- **Batch**: `Clipboard.SetText` + synthetic `Ctrl+V`. Leading space prepended to avoid word-fusion. Prior clipboard contents are cloned and restored ~250ms after the paste; chained dictations reuse the pending saved data so the user's original clipboard survives rapid-fire use. MainWindow's `DeliverAsync` only calls this once the take's own window is verified in front.
- **`CopyToClipboard`**: the text alone, no paste and no restore — for a transcript that can't safely be pasted (its window isn't in front) and for retries. It cancels any pending restore first, which would otherwise overwrite the text 250 ms later.
...
- **Watchdog**: every 30 s, if the hook has seen no key event but `GetAsyncKeyState` says a key was pressed since the last sweep, the hook is reinstalled (a `LowLevelHooksTimeout` drop, or another app — e.g. PowerToys — re-hooking ahead of us). The sweep covers **keyboard vkeys only** (from `VK_BACK`, 0x08): 0x01/0x02/0x04–0x06 are mouse buttons, which the keyboard hook never sees, and including them made every 30 s of clicking-without-typing look like a dead hook (377 needless reinstalls in one session's log). The sweep reads every key's "pressed since" bit without an early exit so a stale bit can't trip a later idle tick.
...
Chirps are procedurally generated sine waves — no asset files — synthesised **once** at startup and played through `UiSoundPlayer`'s persistent output device. Six tones: `Start` (1200 Hz), `Stop` (800 Hz), `Success` (1600 Hz), `Error` (300 Hz), `Dismissed` (520 Hz, quieter — "I saw the press and threw it away", deliberately not the error buzz), and `Warn` (660 Hz, two 90 ms pulses — "delivered, but check it"; pitched off `Dismissed` so the two never blur). Each has a 4 ms attack ramp; the old synth started the envelope at full amplitude, which put a click in front of every chirp.
...
- **`LocalModelGlob` — pin it to the model, not the family.** Presets share `cohere-gguf\` and `ResolveModel` takes the first `EnumerateFiles` match, so a loose glob silently hijacks a sibling preset. `parakeet-*.gguf` matches both Parakeet presets and "rnnt" sorts before "tdt", so they are pinned `parakeet-tdt-*` / `parakeet-rnnt-1.1b-*`; `qwen3-asr-1.7b-*` is pinned the same way against a future 0.6b drop-in. This fails **silently** and presents as a model-quality problem, not a config error. It happened for real to `granite-local`: its `granite-speech-*.gguf` picked up a 2b-plus GGUF downloaded the evening after the preset was committed and ran it for months. Fixing the glob in `CreateDefaults()` alone doesn't reach existing configs, because the default-merge never overwrites fields. A shipped value that turned out wrong also needs a line in `ApiProvider.RepairSupersededDefault`, which rewrites only an exact match of the old value.
- **`LocalBackendHint`** — only needed when GGUF metadata doesn't auto-detect. Cohere needs `"cohere"`, Voxtral 3B `"voxtral"`, Voxtral 4B `"voxtral4b"`, Granite `"granite"`. `granite`, `granite-4.1` and `granite-4.1-plus` are one code path, and the plus variant is detected from the GGUF itself. Qwen3-ASR *does* auto-detect (to `qwen3`) but its preset pins `"qwen3-1.7b"` to document intent. When unsure, pin it: it costs nothing and survives upstream detection changes.
- **`BiasMechanism`** — `"hotwords"` for CrispASR, but the name covers two unrelated mechanisms. On Parakeet it is a **CTC/TDT trie** (weak; garbles neighbours past boost ~8, so `HotwordsBoost` is off by default). On the speech-LLM backends (Voxtral 3B, Qwen3-ASR, Granite) it is **prompt text**. On Qwen3-ASR that is the only local biasing measured to reliably flip a hard term without collateral damage; Granite flips them but rewrote a correct term. Use `"none"` on Cohere/Voxtral-4B — they accept the field and ignore it. **Measure before believing a no-op claim**: upstream adds prompt splices without fanfare, and Granite's went unnoticed for three months.
...
**A new preset beside one the user already has** (another model on the same service, e.g. `elevenlabs-medical` beside `elevenlabs`, `deepgram-medical` beside `deepgram`) needs no key-copying step. When `LoadConfig` merges it into an existing config, `ApiProvider.InheritFromSibling` copies the API key from a same-kind, same-host provider, preferring the one whose id prefixes the new id. Between ElevenLabs entries it also copies `ScribeKeytermsRaw`, `TagAudioEvents` and `NoVerbatim`. Without this a new cloud preset starts keyless and fails every dictation. The values are copied, not linked, so later edits to either entry stay separate.
...
Route bias terms to the API's **native** field inside the transcriber and set `BiasMechanism` to a descriptive string — for these it is informational, since the transcriber does the routing. Two rules: pass the `ct` you're given to **every** request (it is the take's deadline — `TranscriptionDeadline` — and the only thing that ends a slow call; the shared client's own timeout is a 2 h backstop), and give the cancel path two arms — `catch (OperationCanceledException) when (ct.IsCancellationRequested)` for the deadline, and a plain one for the HttpClient's own timeout, which also arrives as a `TaskCanceledException`. A request that must outlive a cancelled take (cleanup, like Soniox's DELETEs) gets its own short `CancellationTokenSource`.
...
- **"Where did that dictation go?"** Check **↻ Unsent dictations** first: every take that wasn't delivered is there, with its reason, and `debug.log` has the matching `[error]` / `[warn]` line. A take that *was* delivered but only to the clipboard (its window wasn't in front) is in History.
- **`debug.log` is per session.** The previous session's is `debug.previous.log` (one restart back — a second restart overwrites it). Timestamps are time-only; the header line carries the date.
...
- **CrispASR failures are logged with their reason (2026-09).** `CrispAsrServerTranscriber` keeps the last 20 lines each server printed (stdout+stderr, per process) and writes them to `debug.log` with the exit code whenever a server dies during startup, misses the `/health` deadline, is found dead at the next dictation, fails a request (non-2xx is logged with a body preview), or returns empty text. `STATUS_DLL_NOT_FOUND` is named outright with the fix. Before this the output was read and discarded, so the log could only say "did not respond" — or nothing. Search `debug.log` for `CrispAsr(` first.
- **Crash-log entries carry the real exception.** `App.WriteCrash` logs `Exception.ToString()`, so an `UnobservedTaskException` records its inner exceptions and stacks (it used to log only the AggregateException's generic "were not observed" message); `RunSafe` logs the base exception's type and stack, not just its message.
- **A local preset transcribes with the *wrong model*.** `LocalModelGlob` collided with a sibling preset — presets share `cohere-gguf\` and `ResolveModel` takes the first `EnumerateFiles` match. Fails silently and reads as a model-quality problem. Pin the glob to the model, not the family (see [Adding a provider](#adding-a-provider)). `granite-local` ran the 2b-plus GGUF this way from May to September 2026.
- **A hand-added cloud provider gets 401 on every take.** Check `debug.log`'s `Active provider: … (auth=…)` line. ElevenLabs hosts now resolve a blank auth header to `xi-api-key`; any other service needs the right `AuthHeaderName` typed in. Failed takes wait in ↻ Unsent dictations.
...
- **Coverage for more providers.** `TranscriptCoverage` only sees ElevenLabs today (the one provider asked for word timestamps). Deepgram already returns per-word `end` times in its default response and Soniox tokens carry `end_ms`, so each is a small `ITranscriptCoverage` implementation away.
- **`debug.log` and the support bundle carry transcript text** (the `result:` and HTTP-preview lines). That's clinical content in a file the tray offers to zip for Slack/email. Worth a redaction pass (or a "no transcripts in the log" switch) before a bundle leaves the machine.
- From the elevenlabs-web port, not started: a phone link (WhisperInk as a listener on the existing worker endpoints, no worker changes) and a CapsLock-hold hotkey. Both wait on the user's call.
- **Streamed upload for more providers.** `StreamedTranscription` is ElevenLabs-only because it rides `file_format=pcm_s16le_16`. Deepgram and Smallest.ai take the audio as the raw request body with the format in query params, so each could stream the same way.

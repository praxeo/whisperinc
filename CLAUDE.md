...
| File | Purpose |
|------|---------|
| `MainWindow.xaml.cs` | Orchestration: recording state machine (`_recState` tri-state via `Interlocked`), transcription dispatch (one factory call, under the take's deadline), verified-target delivery, the unsent-take retry flow, config load/save, the shared `BuildAppMenu()` tree. ~1970 LOC. |
...
Captures via `MicCapture`, journals the take (`UnsentTakes`), POSTs the in-memory WAV under a per-take deadline, pastes with a leading space into the window the dictation started in (only if it is still in front). Works with every provider. `~/Documents/MyRecordings/temp_audio.wav` is written **after** capture as a fire-and-forget replay/debug artifact only — it is no longer in the recording path (that folder is OneDrive-synced on the desktop, so a sync stall must never be able to stall a dictation). The durable copy of a take is the journal in `%APPDATA%\.WhisperInk\unsent\`, which is not synced.
